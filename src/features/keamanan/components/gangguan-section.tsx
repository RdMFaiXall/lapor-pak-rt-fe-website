...
// Filter gangguan data
const gangguanData = mockKeamananData.filter(d => d.category === 'gangguan')

export function GangguanStatusChart() {
    const statusCount = gangguanData.reduce((acc, curr) => {
        acc[curr.status] = (acc[curr.status] || 0) + 1
        return acc
    }, {} as Record<string, number>)

    const total = gangguanData.length
    ...
    return (
        <div className='bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 h-full min-h-[300px]'>
            <h3 className="text-md font-semibold text-slate-400 mb-6">Status Laporan Gangguan Ketertiban Malam</h3>
...
export function GangguanResolution() {
    const total = gangguanData.length
    const selesaiCount = gangguanData.filter(d => d.status === 'resolved' || d.status === 'closed').length
    const belumSelesaiCount = total - selesaiCount

    const resolvedColor = '#10b981' // emerald-500
    const unresolvedColor = '#eab308' // yellow-500
...
export function GangguanPriorityList() {
    const priorityCount = gangguanData.reduce((acc, curr) => {
    ...
    return (
        <Card className='bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 mt-6'>
            <h3 className="text-md font-semibold text-slate-400 mb-6">Tingkat Prioritas Laporan Gangguan Ketertiban Malam</h3>
